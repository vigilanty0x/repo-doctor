...
GOOD = {"query":"SELECT * FROM items WHERE key = ?","before_plan":["SCAN items"],"after_plan":["SEARCH items USING INDEX idx_key"]}
BAD = {"query":"SELECT 1","before_plan":[None],"after_plan":[None]}

class ContractTests(unittest.TestCase):
    def test_valid_record_builds_domain_artifact(self):
        result = evaluate(GOOD)
        self.assertEqual(result["status"], "passed")
        self.assertTrue(result["plan_diff"]["uses_index"])
